...
stDate CountReturnDateFromVecation(stDate StartDay,  short VecationDays)
{
	stDate ReturnDate = StartDay;
	while (VecationDays)
	{
		if (!isWeekEnd(ReturnDate))
			VecationDays--;
		ReturnDate = IncreaseDayByOneDay(ReturnDate);
	}
	return ReturnDate;
}
...
int main()
{
	...
	short VecationDays = ReadNum("\nPlease, enter vecation Days : ");

	cout << "Return Date : " << ConvertDateToText(CountReturnDateFromVecation(VecationStart, VecationDays));
